...
async function refreshAccessToken(event: H3Event) {
  ...
  setCookie(event, STORAGE_KEYS.ACCESS_TOKEN, res.data.access_token, {
    ...COOKIE_BASE, secure: true, maxAge: 60 * 15,
  })
  if (res.data.refresh_token) {
    setCookie(event, STORAGE_KEYS.REFRESH_TOKEN, res.data.refresh_token, {
      ...COOKIE_BASE, secure: true, maxAge: 60 * 60 * 24 * 7,
    })
  }
...
type Init = { method?: string; body?: any; query?: Record<string, any>; headers?: Record<string, string> }
...
export async function callNestWithBearer<T = unknown>(
  ...
  const attempt = (token: string, csrfOverride?: string) => {
    ...
    const headers: Record<string, string> = {
      ...
      ...(baseHeaders['x-csrf-token'] || baseHeaders['X-CSRF-Token'] ? {} : (csrf ? { 'x-csrf-token': csrf } : {})),
    }

    const cookieFromInit = baseHeaders['cookie'] || baseHeaders['Cookie']
    const cookieParts: string[] = []
    if (cookieFromInit) cookieParts.push(String(cookieFromInit))
    if (csrf && !/\bcsrf_token=/.test(cookieFromInit || '')) cookieParts.push(`csrf_token=${csrf}`)
  ...
  try {
    ...
    if (!access) throw createError({ statusCode: 401, statusMessage: 'No access token' })
    ...
    if (status === 403) {
      ...
      setCookie(event, STORAGE_KEYS.CSRF_TOKEN, newCsrf, { httpOnly: false, sameSite: 'lax', path: '/' })
