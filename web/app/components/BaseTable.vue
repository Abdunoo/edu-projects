...
const emit = defineEmits<{
  ...
  (
    e: "sort",
    payload: { key: string | null; dir: "asc" | "desc" | "none" | null },
  ): void;
}>();
...
const startIdx = computed(() => {
  ...
  return Math.min(
    (pageVal.value - 1) * perPageVal.value + 1,
    totalRowsVal.value,
  );
});
const endIdx = computed(() =>
  Math.min(pageVal.value * perPageVal.value, totalRowsVal.value),
);
...
<template>
  <!-- Shell -->
  <div
    class="bg-white dark:bg-zinc-950 rounded-2xl ring-1 ring-zinc-200 dark:ring-zinc-800 shadow-sm overflow-hidden"
  >
    <!-- Toolbar -->
    <div
    ...
    >
      <div class="min-w-0">
        <h2
          v-if="title"
          class="truncate text-base font-semibold text-zinc-900 dark:text-zinc-100"
        >
          {{ title }}
        </h2>
      </div>
    ...
    <div class="overflow-x-auto" :aria-busy="loadingVal">
      <table class="min-w-full" :aria-label="title || 'Data table'">
        <thead
          :class="[
            'border-y border-zinc-200 dark:border-zinc-800 bg-zinc-50/80 dark:bg-zinc-900/70',
            stickyHeader
              ? 'sticky top-0 z-10 supports-[backdrop-filter]:backdrop-blur-md'
              : '',
          ]"
        >
          <tr>
            <th
              v-if="showIndex"
              class="px-4 py-3 text-[11px] font-semibold uppercase tracking-wide text-zinc-600 dark:text-zinc-300 text-left w-12"
            >
              #
            </th>
            <th
              ...
              :class="[
                'px-4 py-3 text-[11px] font-semibold uppercase tracking-wide text-zinc-600 dark:text-zinc-300',
            ...
            >
              ...
              >
                ...
                <Icon
                  v-if="
                    sortState.key === String(col.key) && sortState.dir === 'asc'
                  "
        ...
        <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
          ...
          <tr v-if="loadingVal">
            <td
              :colspan="columns.length + (showIndex ? 1 : 0)"
              class="p-10 text-center text-zinc-500 dark:text-zinc-400"
            >
          ...
          <tr v-else-if="!rowsVal?.length">
            ...
            >
              <div
                class="flex flex-col items-center gap-2 text-zinc-500 dark:text-zinc-400"
              >
                <Icon
                  name="i-lucide-database"
                  class="h-6 w-6 text-zinc-400 dark:text-zinc-500"
                />
          ...
          <template v-else>
            <tr
              ...
              :class="[
                stripedRows
                  ? 'odd:bg-zinc-50/60 even:bg-white dark:odd:bg-zinc-900/40 dark:even:bg-zinc-950'
    ...
    <!-- Footer / Pagination -->
    <div
      class="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between bg-white/60 dark:bg-zinc-950/60"
    >
      ...
      <div class="order-1 flex items-center gap-3 sm:order-2">
        <label
          class="hidden text-sm text-zinc-600 dark:text-zinc-300 sm:inline"
          for="rows-per-page"
          >Rows per page</label
        >
        <select
          ...
          "
        >
          <option v-for="opt in [10, 20, 50, 100]" :key="opt" :value="opt">
            {{ opt }}
          </option>
        </select>
