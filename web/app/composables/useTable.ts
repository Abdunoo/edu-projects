...
export function useTable<T>(params: {
  ...
  const filters = reactive<FilterCondition[]>({ ...(params.initialFilters ?? ({} as FilterCondition[])) })
  ...
  function resetFilters(next?: Partial<FilterCondition[]>) {
    for (const k of Object.keys(filters) as (keyof FilterCondition[] & string)[]) {
      ;(filters as Record<string, any>)[k] = undefined
    }
    Object.assign(filters as Record<string, any>, (next ?? {}) as Record<string, any>)
    page.value = 1
  }
  ...
  function toCSV(opts?: { columns?: { key: keyof T; label: string }[] }) {
    const columns =
      opts?.columns ??
      (Object.keys(rows.value[0] ?? {}) as (keyof T)[]).map((k) => ({ key: k, label: String(k) }))
    const header = columns.map((c) => '"' + String(c.label).replace(/"/g, '""') + '"').join(',')
    const body = rows.value
      .map((r) =>
        columns
          .map((c) => {
            const val = (r as unknown as Record<string, unknown>)[c.key as unknown as string]
  ...
  watch([page, perPage, () => JSON.stringify(filters), () => JSON.stringify(sort.value)], load, {
    immediate: true,
  })
