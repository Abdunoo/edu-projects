    ...
    <div class="page-content">
      <form class="space-y-4 max-w-2xl" @submit.prevent="onSubmit">
        ...
        <div>
          ...
          <input v-model="form.classId" type="text" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Class ID">
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <NuxtLink to="/enrollments" class="inline-flex items-center rounded-md border border-gray-300 px-4 py-2 text-gray-700 hover:bg-gray-50">Cancel</NuxtLink>
        </div>
      </form>
    </div>
  </div>
...
try {
  const { data } = await $api<IBaseApiResponse<IEnrollment>>(`${ENDPOINTS.ENROLLMENTS.BASE}/${recId}`, { method: 'GET' })
  dataItem = data
} catch (e) {
  fetchError = e
}
...
const editSchema = enrollmentSchema.partial({ createdAt: true, updatedAt: true })
...
const onSubmit = async () => {
  ...
  try {
    ...
    await $api<IBaseApiResponse<IEnrollment>>(`${ENDPOINTS.ENROLLMENTS.BASE}/${recId}`, {
      method: 'PUT',
      body: payload,
    })
    ...
    const message = (err as { data?: { message?: string } })?.data?.message || 'Failed to update enrollment'
    toast.error(message)
  } finally {
