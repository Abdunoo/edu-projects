<template>
  <div class="page-container">
    <header class="page-header">
      ...
      <p class="page-description">Update the student information</p>
    </header>
...
// Fetch student
let studentData: IStudent | null = null
let studentError: unknown = null
try {
  const { data }  = await $api<IBaseApiResponse<IStudent>>(`${ENDPOINTS.STUDENTS.BASE}/${studentId}`, { method: 'GET' })
  studentData = data
...
// Initialize form after data resolves
if (studentError) {
...
const editStudentSchema = studentSchema.partial({ createdAt: true, updatedAt: true })
...
const onSubmit = async () => {
  ...
  try {
    const { success, data, errors } = validateForm(editStudentSchema, form)
    if (!success) {
    ...
    await $api<IBaseApiResponse<IStudent>>(`${ENDPOINTS.STUDENTS.BASE}/${studentId}`, {
      method: 'PUT',
      body: payload,
    })
    ...
    const message = (err as { data?: { message?: string } })?.data?.message || 'Failed to update student'
    toast.error(message)
  } finally {
