...
const table = useDataTable<IStudent>({
  ...
  fetcher: async (payload: ListRequestPayload) => {
    const response = await $api<IListResponse<IStudent>>(
    ...
    const d = (response.data ?? {}) as Partial<
      IListResponse<IStudent>["data"]
    > & { meta?: { totalRows?: number } };
    const rows: IStudent[] = Array.isArray(d?.rows)
      ? (d.rows as IStudent[])
      : [];
...
watch([nameQuery, activeQuery], () => {
  ...
  if (
    ...
    (activeQuery.value === "true" || activeQuery.value === "false")
  ) {
...
const columns = [
  ...
  },
  {
    ...
    label: "DATE OF BIRTH",
    sortable: true,
    formatter: (value: IStudent["dob"]) =>
      (value ? formatDate(String(value)) : "-") as string,
  },
  {
    key: "guardianContact",
  ...
  },
  {
    ...
    label: "CREATED",
    sortable: true,
    formatter: (value: IStudent["createdAt"]) =>
  ...
  },
  {
    ...
    label: "LAST UPDATED",
    sortable: true,
    formatter: (value: IStudent["updatedAt"]) =>
      (value ? formatDateTime(value) : "-") as string,
  },
  {
    key: "actions",
    ...
    >
      <template #toolbar>
        <div class="flex items-center gap-2">
          ...
          >
            <option v-for="opt in activeItems" :key="opt.id" :value="opt.id">
              {{ opt.label }}
            </option>
          </select>
          <button
      ...
      <template #cell:isActive="{ row }">
        <span
          :class="[
            (row as IStudent).isActive
              ? 'text-green-600 bg-green-100 px-2 py-1 rounded-lg'
              : 'text-amber-600 bg-amber-100 px-2 py-1 rounded-lg',
          ]"
          >{{ (row as IStudent).isActive ? "Active" : "Inactive" }}</span
        >
      </template>
    </BaseTable>
  </div>
