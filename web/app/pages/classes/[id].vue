...
try {
  const { data } = await $api<IBaseApiResponse<IClass>>(`${ENDPOINTS.CLASSES.BASE}/${recId}`, { method: 'GET' })
  dataItem = data
} catch (e) {
  fetchError = e
}
...
const onSubmit = async () => {
  ...
  try {
    ...
    const message = (err as { data?: { message?: string } })?.data?.message || 'Failed to update class'
    toast.error(message)
  } finally {
