    ...
    >
      <div class="flex items-start justify-between gap-4 p-6">
        <div class="flex items-center gap-3">
          ...
          >
            <i
              class="i-lucide-user-cog text-gray-600 dark:text-gray-300"
              aria-hidden="true"
            ></i>
          </div>
          <div>
...
definePageMeta({
  requiresAuth: true,
  permissionsAny: [Permission.USER_UPDATE],
});
...
const updateUserSchema = userSchema.partial({
  ...
  createdAt: true,
  updatedAt: true,
  password: true, // Password is optional for updates
});
...
try {
  loading.value = true;
  const response = await $api<IBaseApiResponse<IUser>>(
    `${ENDPOINTS.USERS.BASE}/${userId.value}`,
    { method: "GET" },
  );
  ...
  uiState.name = userData.name;
  uiState.email = userData.email;
  uiState.roleId = String(userData.roleId);
  uiState.isActive = userData.isActive;
} catch (err: unknown) {
  const message =
...
const onSubmit = async () => {
  ...
  try {
    const candidate: Omit<IUser, "password" | "createdAt" | "updatedAt"> = {
      id: userId.value,
      name: uiState.name,
      email: uiState.email,
    ...
    const { success, data, errors } = validateForm(updateUserSchema, candidate);
    if (!success) {
      formErrors.value =
    ...
    await $api<IBaseApiResponse<IUser>>(
      `${ENDPOINTS.USERS.BASE}/${userId.value}`,
      {
        method: "PUT",
        body: payload,
